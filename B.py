...
RETURN venueName, year, topPapers
ORDER BY venueName, year;
    """
run_query(query)
...
WHERE v.type = "journal" AND cited.year IN citedYears AND citing.year = citingYear
WITH v, citedYears, COUNT(citing) AS totalCitations

MATCH (v)<-[:PUBLISHED_IN]-(p:Paper)
WHERE p.year IN citedYears
